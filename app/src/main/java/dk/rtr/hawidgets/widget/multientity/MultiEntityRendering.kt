...
import dk.rtr.hawidgets.widget.common.RefreshEntityAction
import dk.rtr.hawidgets.widget.common.WidgetColors
import dk.rtr.hawidgets.widget.common.domainIconResId
import dk.rtr.hawidgets.widget.common.formatDisplayValue
import dk.rtr.hawidgets.widget.common.formatEntityState
import dk.rtr.hawidgets.widget.common.formatRangeValue
import dk.rtr.hawidgets.widget.common.friendlyNameFromJson
import dk.rtr.hawidgets.widget.common.hasOnOffState
import dk.rtr.hawidgets.widget.common.hvacActionFromJson
import dk.rtr.hawidgets.widget.common.isRawValueDomain
import dk.rtr.hawidgets.widget.common.unitFromJson
import dk.rtr.hawidgets.widget.common.isActiveState
import dk.rtr.hawidgets.widget.common.isStale
...
// Fælles indholdshøjde (v0.2.34) for chips OG selve rækkens indhold (v0.3.x-fix) — uden denne på
// rækken driver kun tekst-kolonnen (~33dp) rækkens højde, mens en chip-rækkes chip gjorde DEN
// højere → brugerrapporteret uens rækkehøjde. Én konstant driver begge, så rækkerne altid er ens
// høje, uanset chips. Værdien er nu pr.-widget-valgbar via [RowDensity]; NORMAL = 48dp (uændret).
internal const val CHIP_HEIGHT_DP = 48

/**
 * Pr.-widget række-densitet (brugervalg, [MultiWidgetEntity.rowDensity]). Kun INDHOLDSHØJDEN varierer;
 * den vertikale padding (6dp) og hjørne-radierne er faste, så rækkerne forbliver lige høje og
 * hjørnerne intakte (44dp ≫ 2×10dp). Samlet rækkehøjde = indhold + 2×6dp → 56/60/64dp.
 * 44dp holder 2 linjers 11sp-tekst ved normal fontskala; NORMAL (48) matcher a11y-tap-target-minimum.
 */
internal object RowDensity {
    const val COMPACT = "COMPACT"
    const val NORMAL = "NORMAL"
    const val LARGE = "LARGE"
    const val DEFAULT = NORMAL
    val ALL = listOf(COMPACT, NORMAL, LARGE)

    /** Indholdshøjde i dp for et densitets-nøgleord; ukendt/null → NORMAL (48). */
    fun contentHeightDp(density: String?): Int = when (density) {
        COMPACT -> 44
        LARGE -> 52
        else -> CHIP_HEIGHT_DP // NORMAL + null/ukendt
    }
}
...
@Composable
private fun colorFor(role: ColorRole): ColorProvider = when (role) {
    ...
    ColorRole.CHIP_DIM -> WidgetColors.chipDimText()
    ColorRole.FADED -> WidgetColors.fadedContent()
    ...
    Box(modifier = GlanceModifier.fillMaxSize()) {
        Box(
            modifier = GlanceModifier
                .fillMaxSize()
                .background(WidgetColors.frameBackground())
                .cornerRadius(16.dp)
            ...
            LazyColumn(modifier = GlanceModifier.fillMaxSize()) {
                items(sorted, itemId = { it.slotIndex.toLong() }) { slot ->
                    Column {
                        SlotRow(context, slot, chipsBySlot[slot.slotIndex].orEmpty(), states, contentHeightDp)
...
private fun RefreshStrip(context: Context) {
    ...
    Row(
        modifier = GlanceModifier
            ...
            .background(WidgetColors.refreshOverlay())
            .cornerRadius(16.dp)
...
private fun SlotRow(
    ...
    if (displayEntityId == null) {
        ChipsOnlyRow(context, chips.map { it.toChipData() }, states, contentHeightDp)
        return
    }
    ...
    val bgColor = colorFor(tokens.bg)
    val iconColor = colorFor(tokens.icon)
    val labelColor = colorFor(tokens.label)
    val statusColor = colorFor(tokens.status)
    ...
    val rowContent: @Composable () -> Unit = {
        // Højde altid contentHeightDp (ubetinget, uanset chips.isNotEmpty()) — ellers driver kun
        // ikon/tekst (~33dp) højden på en chipløs række, mens en chip-rækkes tvungne chip-højde gør
        // DEN højere. Med fast højde her bliver alle rækker ens (densitet 44/48/52 → 56/60/64dp total).
        Row(
            modifier = GlanceModifier.fillMaxWidth().height(contentHeightDp.dp),
            verticalAlignment = Alignment.CenterVertically,
        ) {
            if (slot.showIcon) {
                Image(
            ...
            if (chipData.isNotEmpty()) {
                ...
                Row(verticalAlignment = Alignment.CenterVertically) {
                    chipData.forEachIndexed { index, chip ->
                        ...
                        SecondaryChip(context, chip, states, rowHeating = heating && !isUnavailable, contentHeightDp = contentHeightDp)
                    }
                }
            }
        }
...
private fun ChipsOnlyRow(
    ...
    // kræver contentHeightDp) af foroven/forneden (brugerrapporteret v0.3.x-fund). Rammens totale
    // højde bliver dermed indhold + 6dp×2 padding, identisk med en normal række.
    Box(
        modifier = GlanceModifier
            .fillMaxWidth()
        ...
        Row(
            ...
            chips.forEachIndexed { index, chip ->
                SecondaryChip(context, chip, states, rowHeating = false, contentHeightDp = contentHeightDp)
                Spacer(modifier = GlanceModifier.defaultWeight())
            }
        }
    }
...
private fun SecondaryChip(
    ...
    val iconColor = colorFor(tokens.icon)
    val labelColor = colorFor(tokens.label)
    val valueColor = colorFor(tokens.status)
    ...
    Box(
        modifier = withClick(
            GlanceModifier.height(contentHeightDp.dp).background(bgColor).cornerRadius(CHIP_CORNER_DP.dp).then(chipPad),
        ),
        contentAlignment = Alignment.Center,
    ) { chipContent() }
}
