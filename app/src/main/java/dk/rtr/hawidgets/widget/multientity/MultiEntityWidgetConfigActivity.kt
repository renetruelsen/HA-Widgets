...
import dk.rtr.hawidgets.transfer.ConfirmReplaceDialog
import dk.rtr.hawidgets.transfer.ImportError
import dk.rtr.hawidgets.transfer.ImportPickerDialog
import dk.rtr.hawidgets.transfer.ImportPickerItem
import dk.rtr.hawidgets.transfer.TRANSFER_IMPORT_MIME_TYPES
import dk.rtr.hawidgets.transfer.TransferConfig
import dk.rtr.hawidgets.transfer.WidgetTransferIo
import dk.rtr.hawidgets.transfer.importErrorMessage
import dk.rtr.hawidgets.transfer.loadSlotsWithChips
import dk.rtr.hawidgets.transfer.multiTransferConfig
import dk.rtr.hawidgets.transfer.rememberImportLauncher
import dk.rtr.hawidgets.transfer.singleConfigBundle
import dk.rtr.hawidgets.widget.common.AppSettingsHint
import dk.rtr.hawidgets.widget.common.MULTI_ENTITY_DOMAINS
import dk.rtr.hawidgets.widget.common.domainIconResId
import dk.rtr.hawidgets.widget.common.NotConnectedGate
import dk.rtr.hawidgets.widget.common.rememberResumeTick
...
private fun MultiEntityConfigScreen(appWidgetId: Int, onSaved: () -> Unit) {
    ...
    var rowDensity by remember { mutableStateOf(RowDensity.DEFAULT) }
    ...
    LaunchedEffect(resumeTick) {
        ...
        try {
            ...
            val existing = db.multiWidgetDao().get(appWidgetId)
            showRefreshIcon = existing?.showRefreshIcon ?: true
            rowDensity = existing?.rowDensity ?: RowDensity.DEFAULT
            ...
            recoverable = db.multiWidgetDao().getSoftDeleted()
                ...
                .map { multiTransferConfig(loadSlotsWithChips(db, it.appWidgetId), it.showRefreshIcon, it.rowDensity) }
            loadError = null
            loaded = true
        } catch (e: Exception) {
    ...
    if (notConnected) {
        NotConnectedGate(onOpenApp = {
            context.startActivity(Intent(context, dk.rtr.hawidgets.MainActivity::class.java))
        })
        return
    }
    ...
    when (val s = step) {
        Step.ListScreen -> ListScreen(
            ...
            onRowDensityChange = { rowDensity = it },
            ...
            onSave = {
                scope.launch {
                    val db = AppDatabase.get(context)
                    db.multiWidgetDao().upsert(MultiWidgetEntity(appWidgetId, showRefreshIcon = showRefreshIcon, rowDensity = rowDensity))
            ...
            onOpenAppSettings = {
                context.startActivity(
                    Intent(context, dk.rtr.hawidgets.MainActivity::class.java)
                        .putExtra(dk.rtr.hawidgets.MainActivity.EXTRA_OPEN_SETTINGS, true)
                )
            },
            onExport = {
                WidgetTransferIo.shareBundle(
                    context, singleConfigBundle(multiTransferConfig(rows, showRefreshIcon, rowDensity))
                )
            },
