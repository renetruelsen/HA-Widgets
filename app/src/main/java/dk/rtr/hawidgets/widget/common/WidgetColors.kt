...
/**
 * Det aktive tema-tilstands-valg (lys/mørk/system) som en CompositionLocal — leveres reaktivt af
 * [WidgetGlanceTheme] fra `SecureStore.observeThemeSettings()`. De tema-bevidste, IKKE-GlanceTheme-
 * farver ([WidgetColors.frameBackground]/[refreshOverlay]/[chipDimText]/[fadedContent]) læser denne
 * i stedet for `SecureStore` imperativt — ellers re-komponerer de ikke ved et tema-skift (de
 * abonnerer ikke på `GlanceTheme.colors`), så fx refresh-baren "fulgte systemet" indtil næste fulde
 * provideGlance (brugerrapporteret). Via localen udløser selve pref-ændringen deres re-komposition.
 */
internal val LocalWidgetThemeMode = compositionLocalOf { SecureStore.THEME_SYSTEM }
...
object WidgetColors {
    ...
    @Composable
    fun chipDimText(): ColorProvider =
        themed(day = Color(0xFF4A535B), night = Color(0xFFAEB6BA))
    ...
    @Composable
    fun fadedContent(): ColorProvider =
        themed(day = Color(0xFF9AA1A8), night = Color(0xFF7A8085))

    /** Fælles lys/mørk-vælger: tvinger en side ved forced light/dark (så farven matcher
     * GlanceTheme, som også tvinges), ellers en dynamisk day/night-provider. Læser
     * [LocalWidgetThemeMode] (reaktiv) — se dennes doc for hvorfor ikke SecureStore imperativt.
     * Samme mønster som [frameBackground]/[refreshOverlay]. */
    @Composable
    private fun themed(day: Color, night: Color): ColorProvider =
        when (LocalWidgetThemeMode.current) {
     ...
     */
    @Composable
    fun frameBackground(): ColorProvider {
        val day = Color(0x1F1C1B1F)
        val night = Color(0x1FE6E1E5)
        return when (LocalWidgetThemeMode.current) {
            SecureStore.THEME_LIGHT -> ColorProvider(day)
            SecureStore.THEME_DARK -> ColorProvider(night)
            else -> androidx.glance.color.ColorProvider(day = day, night = night)
        }
     ...
     */
    @Composable
    fun refreshOverlay(): ColorProvider {
...
fun WidgetGlanceTheme(context: Context, content: @Composable () -> Unit) {
    ...
    // Leverer themeMode reaktivt til de IKKE-GlanceTheme farve-funktioner (frameBackground/
    // refreshOverlay/chipDimText/fadedContent) — se [LocalWidgetThemeMode]. Uden dette re-komponerede
    // de ikke ved et tema-skift (kun GlanceTheme.colors-læserne gjorde).
    CompositionLocalProvider(LocalWidgetThemeMode provides settings.themeMode) {
